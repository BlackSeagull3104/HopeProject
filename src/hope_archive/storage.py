"""Persistence of minimized source-shaped recovery data, never HTTP captures."""

import json
from pathlib import Path


def save_raw_response(path: Path, body: bytes, *, kind='diary') -> None:
    from .privacy import minimized_response
    path.parent.mkdir(parents=True, exist_ok=True)
    # Exclusive creation preserves earlier evidence, including failed responses.
    with path.open("xb") as stream:
        stream.write(json.dumps(minimized_response(body, kind), ensure_ascii=False).encode('utf-8'))
