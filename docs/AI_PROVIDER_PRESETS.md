...
Verified against official provider documentation on 2026-09-15. Presets are suggestions, not an exhaustive list or a promise of account access. Custom model identifiers are always supported. Credentials are kept in Windows Credential Manager and never returned by metadata or stored in browser storage. Model discovery is an explicit read-only action; it does not submit OCR images, diary text or generation requests.
